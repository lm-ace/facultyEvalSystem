            ...
            <div x-show="showEvaluation" x-cloak x-transition:enter="transition ease-out duration-700" class="space-y-6 md:space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                    ...
                    <div x-data="{ hover: false }" 
                         x-init="if(window.innerWidth < 768) hover = true"
                         @mouseenter="hover = true" 
                         @mouseleave="if(window.innerWidth >= 768) hover = false"
                         class="relative bg-white rounded-[1.5rem] shadow-md transition-all duration-500 overflow-hidden flex flex-col border border-gray-100 h-full group"
                         :class="hover ? '-translate-y-1 md:-translate-y-3 shadow-xl ring-2 ring-[#800000]/10' : 'translate-y-0'">
                        ...
                        <div x-show="hover" 
                             ...
                             class="absolute inset-0 z-30 flex flex-col items-center justify-center p-6 text-center bg-[#800000]/60 backdrop-blur-[2px]">
                            ...
                            <div class="transform transition-all duration-300" :class="hover ? 'scale-100 translate-y-0' : 'scale-90 translate-y-4'">
                                <button onclick="showEvaluationModal('{{ $subject->first_name }} {{ $subject->last_name }}', '{{ $subject->subject_code }}', {{ $subject->offering_id }})"
                                    class="px-6 py-3 bg-white text-[#800000] font-black rounded-xl shadow-2xl uppercase text-[10px] md:text-[11px] tracking-widest flex items-center gap-2 md:gap-3 active:scale-95 transition-transform hover:bg-gray-50">
                        ...
                        <div class="h-56 md:h-64 overflow-hidden bg-gray-100 relative">
                             <img src="{{ asset('images/' . $subject->profile_picture) }}" 
                                 class="w-full h-full object-cover object-top transition-transform duration-700 group-hover:scale-105 {{ $subject->is_evaluated ? 'grayscale' : '' }}"
                                 onerror="this.src='https://ui-avatars.com/api/?name={{ urlencode($subject->first_name . ' ' . $subject->last_name) }}&background=random&size=500'">
                        </div>
